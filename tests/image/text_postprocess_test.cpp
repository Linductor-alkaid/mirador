...
/// Gray byte grid with explicit stride control (plain aggregate by design).
struct Map {
...
[[nodiscard]] uint8_t& at(Map& map, int32_t x, int32_t y) {
    return map.pixels[static_cast<size_t>(y) * static_cast<size_t>(map.stride) + static_cast<size_t>(x)];
}

[[nodiscard]] ImageView map_view(const Map& map) {
    ImageView v;
    v.data = reinterpret_cast<const std::byte*>(map.pixels.data());
    v.width = map.width;
    v.height = map.height;
    v.row_stride_bytes = map.stride;
...
TEST(DbPostprocess, RecoversExpandedBoxWithMeanScore) {
    ...
    const auto boxes = mirador::db_postprocess_aabb(map_view(map), DbPostprocessParams{});
...
TEST(DbPostprocess, ScanOrderAndMultipleBoxes) {
    ...
    DbPostprocessParams const params;
    const auto boxes = mirador::db_postprocess_aabb(map_view(map), params);
...
TEST(DbPostprocess, BoxCapFailsExplicitlyAndClampsExpansion) {
    ...
    params.max_boxes = 1;
    const auto capped = mirador::db_postprocess_aabb(map_view(map), params);
...
TEST(DbPostprocess, ExplicitErrors) {
    ...
    ASSERT_EQ(mirador::db_postprocess_aabb(map_view(map), bad_threshold).status().code(), ErrorCode::kInvalidArgument);

    DbPostprocessParams bad_budget;
    bad_budget.work_budget_bytes = 16;
    ASSERT_EQ(mirador::db_postprocess_aabb(map_view(map), bad_budget).status().code(), ErrorCode::kInvalidArgument);

    // A bitmap beyond the (otherwise valid) budget fails explicitly.
    Map const big = make_map(128, 128, 128);
    DbPostprocessParams tight_budget;
    tight_budget.work_budget_bytes = 1024;  // bitmap needs 2048 bytes
    ASSERT_EQ(mirador::db_postprocess_aabb(map_view(big), tight_budget).status().code(), ErrorCode::kBudgetExceeded);
    ...
    std::vector<uint8_t> rgba(256U, 0);
...
TEST(DbPostprocess, StridePaddingDoesNotChangeResults) {
    ...
    for (int32_t y = 0; y < 16; ++y) {
        for (int32_t x = 0; x < 20; ++x) {
            at(padded, x, y) = at(tight, x, y);
        }
    }
    const auto first = mirador::db_postprocess_aabb(map_view(tight), DbPostprocessParams{});
    const auto second = mirador::db_postprocess_aabb(map_view(padded), DbPostprocessParams{});
...
TEST(ContourBoxes, ExactBoundsInScanOrder) {
    ...
    ContourBoxParams const params;
    const auto boxes = mirador::recover_contour_boxes(map_view(map), params);
    ASSERT_TRUE(boxes.ok()) << boxes.status().message();
    ASSERT_EQ(boxes.value().size(), 2U);
...
TEST(ContourBoxes, EightConnectivityJoinsDiagonalsAndThresholdGates) {
    Map map = make_map(20, 20, 20);
    at(map, 2, 2) = 255;
    at(map, 3, 3) = 255;  // diagonal neighbor: one component with 8-connectivity
    at(map, 10, 2) = 10;  // below the default threshold of 1? no: >= 1 -> foreground

    ContourBoxParams diagonal_params;
    diagonal_params.foreground_threshold = 1;
    const auto boxes = mirador::recover_contour_boxes(map_view(map), diagonal_params);
    ASSERT_TRUE(boxes.ok());
    ASSERT_EQ(boxes.value().size(), 2U);

    ContourBoxParams gated;
    gated.foreground_threshold = 32;  // value 10 no longer counts
    const auto gated_boxes = mirador::recover_contour_boxes(map_view(map), gated);
...
TEST(ContourBoxes, ExplicitErrors) {
    ...
    bad.max_boxes = 0;
    ASSERT_EQ(mirador::recover_contour_boxes(map_view(map), bad).status().code(), ErrorCode::kInvalidArgument);

    ContourBoxParams capped;
    capped.max_boxes = 1;
    fill_rect(map, 5, 5, 2, 2, 255);  // second component
    ASSERT_EQ(mirador::recover_contour_boxes(map_view(map), capped).status().code(), ErrorCode::kBudgetExceeded);
}
