...
/// Test-owned gray buffer with an explicit (possibly padded) row stride.
struct GrayImage {
    std::vector<uint8_t> pixels;
    ImageView view;
};

[[nodiscard]] uint8_t& at(GrayImage& image, int32_t x, int32_t y) {
    const auto stride = static_cast<size_t>(image.view.row_stride_bytes);
    return image.pixels[static_cast<size_t>(y) * stride + static_cast<size_t>(x)];
}

[[nodiscard]] GrayImage make_image(int32_t width, int32_t height, int64_t stride, uint8_t fill) {
    GrayImage image;
    image.pixels.assign(static_cast<size_t>(stride) * static_cast<size_t>(height), fill);
    image.view.data = reinterpret_cast<const std::byte*>(image.pixels.data());
...
TEST(SegmentGrowing, UniformImageYieldsNoSegments) {
    GrayImage const image = make_image(64, 64, 64, 128);
    SegmentGrowingLineDetector detector;
    const auto detected = detector.detect(image.view, LineDetectRequest{}, {});
...
TEST(SegmentGrowing, InputValidationErrors) {
    SegmentGrowingLineDetector detector;
    GrayImage image = make_image(64, 64, 64, 0);
    draw_horizontal_line(image, 20, 5, 58, 220);

    // Wrong format (a structurally valid RGB view).
    const std::vector<uint8_t> rgb_pixels(192U, 0);
    ImageView color;
...
TEST(SegmentGrowing, ContextAndBudgetErrors) {
    SegmentGrowingLineDetector detector;
    GrayImage image = make_image(64, 64, 64, 0);
    draw_horizontal_line(image, 20, 5, 58, 220);
