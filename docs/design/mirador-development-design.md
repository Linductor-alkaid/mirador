...
针对“图标识别”的视觉索引应作为独立能力，而不是与推理结果缓存混在一起。它可以按层级使用精确哈希、感知哈希、颜色/边缘描述子、模板匹配，未来再允许注入 Embedder Backend。查询输出是候选列表、距离、阈值和证据类型，而不是直接宣称语义相同。上层只有在距离、尺度和上下文均满足策略时才复用既有标签。M3 落地为 `mirador::cache` 的有界 `VisualIndex`（精确内容哈希、dHash 感知哈希、模板 NCC 三层证据）；指纹归一化由 `mirador::image` 经 core 的 `VisualPatchFingerprint` 契约提供，Embedder 层按 `POST-04` 延后（`DEC-014`）。
...
输入包含图像、ROI、语言提示、最小置信度、最长边、是否需要逐字符信息和输出坐标空间。输出包含 UTF-8 文本、多边形或矩形、置信度以及可选字符级结果。上层若只需要查找某个文字，可以在结果层做匹配，不应要求 OCR Backend 理解 Agent 意图。M2 冻结的 `OcrRequest`（`DEC-012`）不含逐字符开关与字符级结果字段；它们随首个真实需要的 Backend 引入，避免为不存在的消费者固化 schema。M3 交付的参考后处理组件（DB AABB 后处理、轮廓框恢复、行合并、文本规范化）落在 `mirador::image`，参考适配范围见 `DEC-014`。
...
对于终端 UI，建议默认以 320 或 416 像素输入产生粗候选，在目标过小或置信度不足时只对候选 ROI 进行高分辨率细化，而不是持续提高整屏输入分辨率。OmniParser `icon_detect_v3`、GPA-GUI-Detector 或其他模型只应作为评测候选和外部 Backend 示例。模型许可证、数据来源和权重再分发义务属于具体适配包及最终产品的发布审查，不应让 Mirador Core 与某个模型绑定。letterbox、NMS、类别过滤与小目标 crop-refine 组合器已随 M3 落地于 `mirador::image`（`DEC-014`）。

## 15. ELSED 与传统视觉能力

ELSED 代表 Mirador 与普通“AI 推理封装库”的差异：低负载终端视觉经常可以通过传统几何和图像算法直接解决，不需要启动模型。`LineDetector` 接收灰度图或 ROI，输出统一的 `LineSegmentSet`，并允许实现选择 ELSED、OpenCV LSD 或其他算法。若 ELSED 作为源码依赖引入，应放在可选模块中并审查其许可证，不进入最小核心构建。M3 交付 `LineDetector` SPI 与一方确定性线段检测器（梯度区域生长 + 闭式拟合）；ELSED（Apache-2.0）作为可选适配保留引入窗口，集成方式见 `DEC-009`。
...
M4 冻结补充（`DEC-010`）：阶段一的证据关联门控为 IoU 与包含关系（加类别/文本兼容谓词），中心距离仅作为 trace 观测量记录、不单独触发合并；融合输入坐标空间为 kFrame 与 kOriented，kDisplay 待 M5 平台适配定义变换来源后开放。稳定 ID 采用门控后贪心一对一匹配起步，分裂/合并做事件识别；置信度为来源权重与几何一致性的显式规则，不伪装成概率。
...
M4 冻结补充：`mirador::render` 链接恰为 `mirador::fusion`（`DEC-013`），不直接依赖 image/cache；SoM 渲染接受 rotation k0 的单平面 8 位背景，输出自有的 RGB8 标记图像与 `mark_id -> stable_id` 映射，标签为确定性点阵数字与固定调色板（字体/缩放等可配置项随需求扩展）；网格工具只提供纯几何划分与坐标回映，像素裁剪仍由调用方经 `mirador::image` 完成。
