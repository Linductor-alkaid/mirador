...
> 前置：M2
> 发布点：`v0.1.0-beta.3`（tag 打在 PR #8 合并提交）
> 更新日期：2026-09-15
...
- [x] LineDetector SPI 可被伪实现注入并经基类指针调用；一方检测器在合成图（水平/
  垂直/对角线段、奇数尺寸、非连续 stride）上确定性检出，输出位稳定。
- [x] 线段过滤：角度、长度、共线合并的包含/排除边界均有正负用例。
- [x] letterbox 变换与像素操作一致（缩放/居中偏移按实际 rw/rh 推导），与
  `make_letterbox` 连续映射在 1px 容差内一致；填充区域与预算上限有负向用例。
- [x] NMS：重叠抑制、分数排序稳定性、类别感知隔离；类别过滤：置信度与类别边界用例。
- [x] DB 后处理/轮廓框恢复：合成概率图上确定性恢复框，盒数超限显式报错、缓存输入
  不被修改（`RULE-04`/`RULE-06`）。
- [x] crop-refine：候选选择边界、坐标往返（恢复框与解析期望一致，`DOD-03`）、
  取消/超时零 Backend 调用、预算超限。
- [x] 视觉索引：三层证据各自的命中/不命中边界（阈值上下）、相同内容精确命中、扰动
  内容感知命中、字节预算逐出与单条目超预算显式错误（`DOD-04` 思路的负向路径）。
- [x] 架构测试演进后：geometry 链接闭包仅 core + 标准库；公共头与 `src/` 无第三方与
  线程令牌；CI 全部 job 运行。
- [x] `DEC-009`/`DEC-014` 冻结为 Accepted；设计文档 §12/§14/§15 按冻结契约同步。
...
- 限制与后续：NDK 侧仍仅 configure/build 验证，设备侧测试按计划 M5 补跑；线段检测器
  检出质量与性能按 `RISK-2026-09` 在 M5 基准收口。

2026-09-15：PR #8 经用户授权合并（merge commit），发布点 `v0.1.0-beta.3` 发布：tag 打在
PR #8 合并提交，GitHub Pre-release，发布说明见 CHANGELOG 对应版本段。M3 全部工作项、
退出条件与发布点均已闭合。
