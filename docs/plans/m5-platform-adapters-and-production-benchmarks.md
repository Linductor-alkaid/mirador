...
> 前置：M4
> 发布点：`v0.2.0`（tag 打在 PR #12 合并提交 236df8f）
> 更新日期：2026-09-16
...
2026-09-16：用户授权（「授权」）发布收尾，M5 发布点闭合。

- `v0.2.0` tag 打在 PR #12 合并提交 236df8f，与总计划里程碑索引一一对应；
  GitHub Release 发布说明取自 CHANGELOG `0.2.0` 段（含 `DEC-011` 限定表述与
  兼容性影响节）。状态行与总计划登记同步，无代码与构建影响。
