...
- [x] 融合关联：IoU/包含门控的正负边界、类别不兼容不合并、相邻但语义不同不因距离
  合并；输出确定性（同输入位稳定）；`source_mask`/`evidence_ids` 与 trace 一致。
- [x] 坐标：证据从 kFrame/kOriented 转换到目标空间经方向（0/90/180/270）与奇数尺寸
  矩阵验证（`DOD-03`）；不支持的空间显式拒绝。
- [x] 稳定 ID：小幅位移保留 ID、大幅位移换新 ID、分裂/合并事件与 generation 递增、
  文本相似度参与匹配、确定性平局；`RULE-09`（会话内稳定）有边界用例。
- [x] 会话：`fuse()` 全链路（伪 Backend）端到端、未变化画面复用路径不被破坏、快照
  不可变（发布后读取不受后续 fuse 影响）、取消/超时零 Backend 调用、预算超限显式
  `kBudgetExceeded`。
- [x] SoM：标记映射完整（每个区域一个 mark）、绘制确定性（同输入位稳定）、标签
  遮挡规避确定性、网格划分与坐标回映往返一致；`max_marks` 预算显式错误。
- [x] 架构测试演进后：render 链接恰为 fusion、fusion 闭包仍仅标准库；公共头与
  `src/` 无第三方与线程令牌；CI 全部 job 运行。
- [x] `DEC-010` 冻结为 Accepted；设计文档 §8/§16/§17 按冻结契约同步（如有偏差）。
...
2026-09-16：退出条件复选框簿记补勾（无代码变更）。

- 「测试与退出条件」8 项在 `M4-02`~`M4-08` 验证记录中均已具备证据（6 Linux 预设
  18/18、tsan 经 `setarch -R` 连续稳定、lint 双口径归零、CI run `34892045507`
  10/10 绿、`DEC-010` 已列总计划已生效决策、设计 §16 已含 M4 冻结补充），但
  复选框未随证据同步勾选。逐项核对证据后补勾；后续 `M5-07`/`M5-09` 的并发矩阵、
  fuzz 与终轮 6 预设 + CI 13/13 矩阵对相关项形成再次确认。
